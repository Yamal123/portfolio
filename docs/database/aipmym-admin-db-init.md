...
按以下顺序依次执行 SQL，**不可打乱顺序**（存在外键依赖）：

```
Step 1 → 创建数据库
Step 2 → 创建无依赖的基础表 (admin_user, site_config, user_profile, contact_info)
Step 3 → 创建标签/分类表 (user_tag, skill_cate, project_cate)
Step 4 → 创建业务主表 (skill_info, project_info, visit_stat, admin_log)
Step 5 → 填充基础数据
```

### 注意事项

1. **默认管理员账号**：`admin` / 密码 `123456`
2. **加密方式**：MD5(密码 + 盐值)，盐值为 `aipmym_salt_2026_v1`
3. **首次登录后请立即修改密码**
4. 所有表使用 InnoDB 引擎，字符集 utf8mb4
5. `sort_num` 越小越靠前；`status=1` 为启用/上架/显示
6. 执行前请确认 MySQL 版本 >= 5.7 或 >= 8.0
...
> **账号**: `admin`  
> **密码**: `Admin@2026`（符合规则：10位 + 大写A + 小写dmin + 数字2026）
> **盐值**: `aipmym_salt_2026_v1`  
> **加密方式**: `MD5(密码 + 盐值)`  
> **加密结果**: `MD5('Admin@2026' + 'aipmym_salt_2026_v1')` = `1f184d902ff94e4152f46454e7ed4095`
...
-- 两个值应该相等: 1f184d902ff94e4152f46454e7ed4095
...
- 密码校验：computed_password = password ✅
- site_config：6 条
- user_profile：1 条
- contact_info：1 条
- user_tag：7 条
