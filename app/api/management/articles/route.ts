...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.error('[Management articles GET]', error)
...
async function write(request: Request, update: boolean) {
  ...
  if (!parsed.success) return NextResponse.json({ code: 400, message: '文章数据格式无效' }, { status: 400 })
  try {
    return NextResponse.json({ code: 0, data: await saveArticle(parsed.data, update), message: update ? '文章已更新' : '文章已创建' })
  } catch (error) {
    console.error('[Management articles write]', error)
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ code: 0, message: '文章已删除' })
  } catch (error) {
    console.error('[Management articles DELETE]', error)
